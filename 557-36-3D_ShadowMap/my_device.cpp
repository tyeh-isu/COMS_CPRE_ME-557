...
void MyDevice::_pickPhysicalDevice()
{
    ...
    // For this application, we need push constant size to be more than 2 4x4 matrices
    unsigned int size = _getMaxPushContantSize();
    if (size <= 128)
    {
...
VkSampleCountFlagBits MyDevice::_getMaxUsableSampleCount()
{
    if (m_vkPhysicalDevice != VK_NULL_HANDLE)
    {
...
unsigned int MyDevice::_getMaxPushContantSize()
{
    if (m_vkPhysicalDevice != VK_NULL_HANDLE)
    {
        VkPhysicalDeviceProperties physicalDeviceProperties;
        ...
        return (unsigned int)physicalDeviceProperties.limits.maxPushConstantsSize;
    }
...
bool MyDevice::formatIsFilterable(VkFormat format, VkImageTiling tiling)
{
...
void MyDevice::createImageViewWithInfo(
    const VkImageViewCreateInfo& imageInfo,
    VkImageView& imageView)
{
    if (vkCreateImageView(m_vkDevice, &imageInfo, nullptr, &imageView) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to create image memory!");
    }
}

void MyDevice::createSampler(
    const VkSamplerCreateInfo& sampleInfo,
    VkSampler& sampler)
{
    if (vkCreateSampler(m_vkDevice, &sampleInfo, nullptr, &sampler) != VK_SUCCESS)
    {
