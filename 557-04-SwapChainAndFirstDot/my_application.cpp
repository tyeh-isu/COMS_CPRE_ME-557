...
void MyApplication::_createCommandBuffers()
{
    ...
    for (int i = 0; i < m_vVkCommandBuffers.size(); i++)
    {
        ...
        if (vkBeginCommandBuffer(m_vVkCommandBuffers[i], &beginInfo) != VK_SUCCESS)
        {
        ...
        renderPassInfo.framebuffer = m_mySwapChain.frameBuffer(i);
        ...
        vkCmdEndRenderPass(m_vVkCommandBuffers[i]);
        if (vkEndCommandBuffer(m_vVkCommandBuffers[i]) != VK_SUCCESS)
        {
