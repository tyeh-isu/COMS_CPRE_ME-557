...
{
    // If we only create one buffer
	/* MyBuffer globalUboBuffer
    {
      m_myDevice,
      sizeof(MyGlobalUBO),
      MySwapChain::MAX_FRAMES_IN_FLIGHT,
      VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
      VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, // no host coherience bit because it could be different between frames
      minOffsetAlignment,
    };

    globalUboBuffer.map(); // to enable writting the memory to the buffer*/
    ...
    while (!m_myWindow.shouldClose()) 
    {
        ...
        if (auto commandBuffer = m_myRenderer.beginFrame())
		{
            ...
            MyFrameInfo frameInfo{ frameIndex, frameTime, commandBuffer, camera };

            // update UBO to GPU
            // please note that we haven't actaully use the UBO in the shader yet
            // without changing the shader code
            MyGlobalUBO ubo{};
