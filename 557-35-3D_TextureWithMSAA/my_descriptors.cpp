...
MyDescriptorWriter& MyDescriptorWriter::writeImages(
    uint32_t binding, VkDescriptorImageInfo* imageInfo, uint32_t numImages)
{
    assert(m_myDescriptorSetLayout.m_mapBindings.count(binding) == 1 && "Layout does not contain specified binding");
    ...
    assert(
        bindingDescription.descriptorCount == numImages &&
        "Binding single descriptor info, but binding expects multiple");
    ...
    write.pImageInfo = imageInfo;
    write.dstArrayElement = 0;
    write.descriptorCount = numImages;
